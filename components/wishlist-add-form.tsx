...
type WishlistAddFormProps = {
  onAdd: (input: WishlistAddInput) => void;
  pending?: boolean;
};

export function WishlistAddForm({ onAdd, pending = false }: WishlistAddFormProps) {
  ...
  return (
    <form className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-4" onSubmit={handleSubmit}>
      <Input
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex gap-2">
          {(["must_have", "nice_to_have"] as const).map((key) => (
            ...
            >
              {key === "must_have" ? t("priorityMustHave") : t("priorityNiceToHave")}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-xs text-muted-foreground">
          {t("photosHint", { max: MAX_IMAGES_PER_ITEM, sizeMb: 5 })}
        </p>
        <input
          ref={fileInputRef}
          type="file"
        ...
        {fileError && (
          <p className="text-xs text-destructive">
            {fileError === "invalidType" ? t("imageInvalidType") : t("imageTooLarge")}
          </p>
        )}
        {files.length > 0 && (
